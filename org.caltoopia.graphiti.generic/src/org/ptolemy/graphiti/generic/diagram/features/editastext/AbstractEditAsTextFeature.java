...
import org.ptolemy.graphiti.generic.diagram.features.editastext.IEditAsTextHelper.Context;
...
public class AbstractEditAsTextFeature extends AbstractCustomFeature implements Context {
	...
	@Inject
	protected ActorViewModel actorViewModel;
	...
	protected void execute(EObject bo, int x, int y) {
		editableObject = bo;
		Control control = ((DiagramEditor) getDiagramBehavior().getDiagramContainer()).getGraphicalViewer().getControl();
		...
		editAsTextHelper.editAsText(editableObject, getStructuralFeatures(), composite, x, y);
	}
